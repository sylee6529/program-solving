...
### 딱 나눠 떨어지는 것과 아닌 것을 구분하여 더하고 싶다

- 면, 올림 공식을 사용해보자
```java
answer += (arr[i] / c) + 1;  // 딱 나눠 떨어지는 경우에는 +1 안하고 싶은데...
```

TO-BE
```java
answer += ((arr[i] + c - 1) / c);  // 딱 나눠 떨어지는 경우에는 +1 안하고 싶은데...
```
위와 같이 하면,
- 나머지가 있는 경우는 나눈 값에 +1되고,
- 나머지가 없는 경우에는 나눈 값 그대로가 더해진다!

---

### 성능 요약

메모리: 317908 KB, 시간: 1552 ms
